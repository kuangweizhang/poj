...
class calfflac
{
	...
	public static void main(String[] args) throws IOException
	{
		...
		for (int mid = 0; mid < stringCharLength - longestlength / 2; mid++)
		{
			...
			leftworker = nextChar(leftworker, -1, stringsChars,
					stringCharLength);
			rightworker = nextChar(rightworker, 1, stringsChars,
					stringCharLength);
			if (leftworker == null || rightworker == null)
			{
				continue;
			}
			while (charequal(stringsChars[leftworker],
					stringsChars[rightworker]))
			{
				lengthcounter += 2;
				leftworker = nextChar(leftworker - 1, -1, stringsChars,
						stringCharLength);
				rightworker = nextChar(rightworker + 1, 1, stringsChars,
						stringCharLength);
				if (leftworker == null || rightworker == null)
				{
					break;
			...
			if (lengthcounter > longestlength)
			{
				...
				longeststart = nextChar(leftworker + 1, 1, stringsChars,
						stringCharLength);
				longestend = nextChar(rightworker - 1, -1, stringsChars,
						stringCharLength);
			}
	...
	public static Integer nextChar(int index, int direction, char[] string,
			int length)
	{
